...
class TestEncryptPacket(CryptoTestCase):
    def test_encrypt_should_encrypt_a_packet(self):
        packet = [1] * 32
        expected = [140, 217, 26, 172, 142, 219, 245, 20, 152, 62, 105, 149, 241, 5, 26, 22, 94, 119, 229, 159, 143,
                    133, 148, 52, 137, 162, 65, 73, 199, 95, 78, 201]

        self.assertEqual(crypto.encrypt_packet(bytes(packet)), bytes(expected))

    def test_the_two_blocks_of_the_encrypted_packet_should_change_if_one_of_the_last_bytes_changes(self):
        packet = [random.randint(0, 255) for dummy in range(32)]
        encrypted_unchanged_packet = crypto.encrypt_packet(bytes(packet))

        packet[31] = packet[31]+1 if packet[31] < 255 else 0
...
class TestDecryptPacket(CryptoTestCase):
    def test_decrypt_should_decrypt_a_packet(self):
        packet = [1] * 32
        expected = [189, 111, 42, 174, 34, 203, 31, 103, 171, 214, 178, 148, 192, 215, 167, 11, 188, 110, 43, 175, 35,
                    202, 30, 102, 170, 215, 179, 149, 193, 214, 166, 10]
